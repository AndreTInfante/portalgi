...
![Conventional PCCM breaks down at apertures: because the cubemap is projected against the convex boundaries of the room, the doorway and the room beyond it flatten against the wall, so the reflection no longer matches the scene geometry.](WriteupAssets/pccm_artifact.png)
...
The idea here is simple: we use the generalization of PCCM to arbitrary convex hulls (a well established technique) with one new conceptual element: each hull can have portals (defined as rectangles on faces for the purposes of the current demo) linking it to other hulls. If a ray intersects a given hull, we fetch the matching texel. If it instead hits a portal, we recurse to the linked hull, and so on. This is very similar to how visibility worked in the Build engine back in the day. At the end of the recurse (either hitting the budget or an actual wall), we terminate, sample the last hull’s environment map as usual, and that’s our reflection. 
...
![The reflection stays stable and spatially consistent under motion without sliding, flattening, or distorting.](WriteupAssets/parallax_demo.mp4)

In terms of implementation, we store each cell as a 512x512 octahedral environment map, with pre-filtered radiance stored in the mip chain (standard technique), atlased together (along with light probe data). We have a live set of loaded hulls (you’d want to stream this in a real application, for the demo it’s small enough that it can just be static). We also store a per-cell mask indicating which faces are portals, and which cells they connect to. At render time, if we hit a portal, we check the same ray against that portal’s environment map instead (everything is tracked in world space, so this is trivial to do, no transformation needed). 
...
This technique is significantly more expensive than PCCM. Each hop is roughly 2 atlas taps + 1 extra hull-plane scan. Logically, you'd expect this to be about 2x the cost of normal PCCM in the most common unrolled single-hop configuration, but because so many pixels can early-out, in actual profiling on Quest 3, portal reflections average 1.5x the cost of my PCCM implementation in a given scene, with a range of about 1.1x-1.76x (Though register allocation is properly 2x). 

This isn't nothing, but the visual benefits are pretty significant (you can A/B test it in the demo) and PCCM starts pretty cheap. The demo runs at a locked 60 on my not-that-nice android phone and locked 72 on Quest 3 (90 in most scenes). Compared to what you’d need to do to get the same fidelity via real time cubemaps or ray marching or what have you, it’s a pretty good deal.
...
Not measured in these numbers: static AO / shadow layer regen is a separate half-res offscreen pass, so it is not in the eye-buffer figures above. I measured it on desktop at <= ~1 ms at full-res (4x the pixel count compared to Quest) depending on how many props are moving (0 at rest). I haven't captured this statistic on Quest because it's kind of a pain and the shadow implementation isn't that interesting (see below).

Caveat: the PCCM implementation I'm using for reference is a bit heavier than what you'd see in production in most game engines, since they typically restrict themselves to cuboids, with simpler ray intersection math. Adopting the same restriction would make both PCCM and portal reflection implementations somewhat cheaper. On the flip side, the PCCM implementation doesn't do any of the shader work associated with cubemap cross-fading, which a real application would generally need.
...
I added the courtyard as a nod to this, but even if you want this to be your sole specular lighting model for a whole game, you can still use it in other settings. There's no performance downside to making arbitrarily huge "cells", and if you wanted to do an open world game, you could imbed your indoor structures inside a huge skybox. You'd need to tweak the system a medium amount (mainly adding support for free-standing portals and increasing max portal count a bit, as well as adding in-cell portal streaming), but the conceptual material is the same.
...
PortalIBL is a good idea I had and built a prototype of, and it seems to work pretty well in practice. If you’re a game developer building stuff for lower end systems (especially XR), you should use it to make your games look nicer. I'm putting the demo out under an MIT license. Additionally, if you’re hiring game developers or XR people interested in graphics in the bay area or remote, you should consider hiring me. You can reach me via email at aticper{at}gmail.com, or on Twitter at [@AndreTI](https://twitter.com/AndreTI).
