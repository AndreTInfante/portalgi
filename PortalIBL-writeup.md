...
Caveat: the PCCM implementation I'm using for reference is a bit heavier than what you'd see in production in most game engines, since they typically restrict themselves to cuboids, with simpler ray intersection math, although those implementations would not be able to competently handle the round gallery regardless. If you're willing to restrict yourself to boxes, you could make both the PCCM and portal reflection implementations somewhat cheaper. On the flip side, the PCCM implementation doesn't do any of the shader work associated with cubemap cross-fading, which a real application would generally need.
