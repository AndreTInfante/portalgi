...
export class Lightmapper {
  constructor(renderer, level, textures, opts = {}) {
    ...
    // Proxied statics (statues, furniture) DO cast lightmap shadows despite
    // also contributing capsule AO - directional shadow information exists
    // nowhere else in the pipeline for them, so the mild double-count is the
    // better trade. (They are excluded from the CUBEMAP captures instead,
    // where the capsules are the better representation.)
